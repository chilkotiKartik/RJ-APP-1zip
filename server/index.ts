...
const EXPO_PORT = parseInt(process.env.EXPO_PORT || '8080');
        ...
        error: (_err, _req, res) => {
          if (res && 'status' in res) {
            (res as express.Response).status(502).send(
              '<h3 style="font-family:monospace;padding:32px">Starting Expo web server — refresh in a moment…</h3>'
            );
          }
        },
      },
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  if (!IS_PROD) {
    console.log(`[rj-server] Proxying UI → http://localhost:${EXPO_PORT}`);
  }
});
